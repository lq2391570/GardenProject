...
struct MyCircleViewModel1 {
    var dynamiBassClass:ActivityActivityBassClass!
  
    
    var memModel:ActivityMember {
        return dynamiBassClass.member!
    }
    var headImageStr:String {
        return dynamiBassClass.member?.avatar ?? ""
    }
    var nameAndPositionStr:NSAttributedString {
        let nameAttribute = [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 15)]
        let nameAttribeStr = NSMutableAttributedString(string: "\(dynamiBassClass.member?.name ?? "")  ", attributes: nameAttribute)
        let positionAttribute = [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 13),NSAttributedStringKey.foregroundColor: UIColor.gray]
        let positionAttribeStr = NSMutableAttributedString(string: dynamiBassClass.member?.job ?? "", attributes: positionAttribute)
        nameAttribeStr.append(positionAttribeStr)
        return nameAttribeStr
    }
    var companyStr:String {
        return dynamiBassClass.member?.companyName ?? ""
    }
    var phoneNumStr:String {
        return dynamiBassClass.member?.job ?? ""
    }
...
extension MyCircleViewModel1:MyCircleRespertable {}
extension MyCircleViewModel2:MyCircleRespertable2 {}
