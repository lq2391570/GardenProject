//
//  MyPublishMyPublishbassClass.swift
//
...
public final class MyPublishMyPublishbassClass: NSCoding {
  ...
  public required init(json: JSON) {
    ...
    if let items = json[SerializationKeys.list].array { list = items.map { MyPublishList(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.list = aDecoder.decodeObject(forKey: SerializationKeys.list) as? [MyPublishList]
