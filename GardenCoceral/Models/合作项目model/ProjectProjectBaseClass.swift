...
public final class ProjectProjectBaseClass: NSCoding {
  ...
  public required init(json: JSON) {
    ...
    if let items = json[SerializationKeys.list].array { list = items.map { ProjectList(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.list = aDecoder.decodeObject(forKey: SerializationKeys.list) as? [ProjectList]
