...
//        slider.backgroundColor = _titleSelectedColor;
    //    [self addSubview:slider];
        if (!self.slider) {
            ...
            self.slider.frame = CGRectMake(0, _HeaderH-2, 0, 2);
            self.slider.backgroundColor = _titleSelectedColor;
     ...
     //   CGFloat btnW = titleWidth+20;
        ...
        if (i == self.currentSeleCtIndex) {
            ...
            if (style == CBSegmentStyleSlider) {
                ...
                if (self.currentCenterX == 0) {
                     self.slider.cb_CenterX = btn.cb_CenterX;
                }else{
                      self.slider.cb_CenterX = self.currentCenterX;
                }
...
- (void)titleButtonSelected:(UIButton *)btn {
    self.currentSeleCtIndex = btn.tag - 100;
    self.selectedBtn.selected = NO;
    btn.selected = YES;
    if (_SegmentStyle == CBSegmentStyleSlider) {
        NSNumber* sliderWidth = self.titleWidthArray[btn.tag - 100];
        [UIView animateWithDuration:0.2 animations:^{
            self.slider.cb_Width = sliderWidth.floatValue;
            self.slider.cb_CenterX = btn.cb_CenterX;
            self.currentCenterX = self.slider.cb_CenterX;
        }];
    }else if (_SegmentStyle == CBSegmentStyleZoom) {
        [UIView animateWithDuration:0.2 animations:^{
...
//    }
  ...
  //  [self setContentOffset:CGPointMake(offsetX, 0) animated:YES];
    if (self.titleChooseReturn) {
        self.titleChooseReturn(btn.tag - 100);
    }
}
//  cache title width
