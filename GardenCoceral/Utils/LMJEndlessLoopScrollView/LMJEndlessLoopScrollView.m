...
- (void)buildScrollView{
   ...
   // _scrollView.contentSize      = CGSizeMake(SelfWidth_LMJ *3, SelfHeight_LMJ);
    _scrollView.contentSize      = CGSizeMake(SelfWidth_LMJ , SelfHeight_LMJ*3);
   // _scrollView.contentOffset    = CGPointMake(SelfWidth_LMJ, SelfHeight_LMJ);
...
- (void)tapContentView:(UITapGestureRecognizer *)gesture{
    ...
    if ([self.delegate respondsToSelector:@selector(loopScrollView:didSelectContentViewAtIndex:topOrBottom:)]) {
        if (point.y < 40) {
            [self.delegate loopScrollView:self didSelectContentViewAtIndex:_currentPageIndex topOrBottom:0];
        }else{
            [self.delegate loopScrollView:self didSelectContentViewAtIndex:_currentPageIndex topOrBottom:1];
        }
