...
class EnrollRpCell: UITableViewCell, Reusable {
    
    let disposeBag = DisposeBag()
    var selectLogo = UIImageView()
    ...
    var midLine = UIView()
    var payWay = UILabel()
    var backRedpacket: ((Double)->())?
    var selectedIndex: Int?
    
    var rpArr = [
        RedPacket(1,money: 10,selected: false),
        RedPacket(5,money: 20,selected: false),
        RedPacket(10,money: 50,selected: false),
        RedPacket(20,money: 100,selected: false),
        RedPacket(50,money: 200,selected: false),
        RedPacket(100,money: 500,selected: false)
    ]
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        let itemWidth = (windowWidth-60)/3
        _ = cellLayout.then{
        ...
        _ = moneyText.then{
            contentView.addSubview($0)
            $0.snp.makeConstraints({ (m) in
                m.top.equalTo(rpView.snp.bottom).offset(20)
        ...
        _ = payWay.then{
            contentView.addSubview($0)
            $0.snp.makeConstraints({ (m) in
                m.top.equalTo(midLine.snp.bottom).offset(25)
                m.left.right.equalTo(rpView)
                m.height.equalTo(21)
                m.bottom.equalTo(-10)
            })
    ...
    func dealData() {
        MyService.shareInstance.getEnrollConfig("1", userToken: getCurrentUser().userToken ?? "")
...
extension EnrollRpCell: UICollectionViewDelegate, UICollectionViewDataSource {
    func resetRedPacket() {
        self.moneyText.text = ""
